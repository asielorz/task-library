...
TEST_CASE("Can pass a move only type to a continuation as long as there is only one continuation")
{
	auto task_queue = TaskQueue(1);

	int i = 0;
	auto t = task([]() { return std::make_unique<int>(5); })
...
TEST_CASE("With multiple continuations last one moves")
{
	struct count_copies_t
	{
		count_copies_t(int x, int & c) noexcept : value(x), copies(c) {}
		count_copies_t(count_copies_t const & other) noexcept : value(other.value), copies(other.copies) { copies++; }
		count_copies_t(count_copies_t &&) noexcept = default;
	...
	int j = 0;
	int k = 0;
	auto t = task([&copies]() { return count_copies_t(5, copies); })
		.then(continuation([&i](count_copies_t x) { i = x.value; }, task_queue))
		.then(continuation([&j](count_copies_t x) { j = x.value + 1; }, task_queue))
		.then(continuation([&k](count_copies_t x) { k = x.value - 1; }, task_queue));
